...
std::pair<int, int> getRestrictedStartPos(std::vector<int> subcellArr, int w, int h, int offsetX, int offsetY, int s, Direction8 d) {
    switch (d) {
    case NW:
        if (isValidPos(subcellArr, w, 0+offsetX, 0+offsetY, s)) return std::make_pair(0+offsetX, 0+offsetY);
        return std::make_pair(-1, -1);
    case N:
        for (int i = 0; i < w - s; i++) {
            if (isValidPos(subcellArr, w, i+offsetX, 0+offsetY, s)) return std::make_pair(i+offsetX, 0+offsetY);
        }
        return std::make_pair(-1, -1);
    case NE:
        if (isValidPos(subcellArr, w, w - s +offsetX, 0+offsetY, s)) return std::make_pair(w - s+offsetX, 0+offsetY);
        return std::make_pair(-1, -1);
    case E:
        for (int y = 0; y <= h - s; y++) {
            if (isValidPos(subcellArr, w, w - s+offsetX, y+offsetY, s)) return std::make_pair(w - s+offsetX, y+offsetY);
        }
        return std::make_pair(-1, -1);
    case SE:
        if (isValidPos(subcellArr, w, w - s+offsetX, h - s+offsetY, s)) return std::make_pair(w - s+offsetX, h - s+offsetY);
        return std::make_pair(-1, -1);
    case S:
        for (int x = 0; x <= w - s; x++) {
            if (isValidPos(subcellArr, w, x+offsetX, h - s+offsetY, s)) return std::make_pair(x+offsetX, h - s+offsetY);
        }
        return std::make_pair(-1, -1);
    case SW:
        if (isValidPos(subcellArr, w, 0+offsetX, h - s+offsetY, s)) return std::make_pair(0+offsetX, h - s+offsetY);
        return std::make_pair(-1, -1);
    case W:
        for (int y = 0; y <= h - s; y++) {
            if (isValidPos(subcellArr, w, 0+offsetX, y+offsetY, s)) return std::make_pair(0+offsetX, y+offsetY);
        }
        return std::make_pair(-1, -1);
    }
}
...
std::vector<bool> preprocessRestrictedValidPositions(std::vector<int> subcellArr, int w, int s, int offsetX, int offsetY) {
    std::vector<bool> retArr(w*w*9, false);//The array being returned. Initially assume all positions are impassible
    for (int i = 0; i < w; i++) {//x position
        for (int j = 0; j < w; j++) {//y position
            //If this position is passable by the agent, can set it to true
            if (isValidPos(subcellArr, w, i+offsetX, j+offsetY, s))
                retArr[((j+offsetY) * w) + i+offsetX] = true;
        }
    }
    return retArr;
}
...
bool isPathBetween(Direction8 from, Direction8 to, std::shared_ptr<GridData> g, int indexAt, int indexTo, int s) {
    ...
    std::pair<int, int> startPos = getRestrictedStartPos(combinedCells, g->subWidth, g->subWidth, s, 
            startTile.first*g->subWidth, startTile.second*g->subWidth, getOppositeDirection(from));
    printf("SX: %i, SY: %i\n", startPos.first, startPos.second);
    if (startPos == std::make_pair(-1, -1)) return false; //Check that it is valid
    std::pair<int, int> endPos = getRestrictedStartPos(combinedCells, g->subWidth, g->subWidth, s, 
        startTile.first*g->subWidth, startTile.second*g->subWidth, getOppositeDirection(to));
    ...
    std::vector<bool> prepArray = preprocessRestrictedValidPositions(combinedCells, dimensions.first, s, startTile.first*g->subWidth, startTile.second*g->subWidth);//Get the valid positions in the array
